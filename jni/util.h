...
void malloc_aligned(aligned_pointer_t * ptr, unsigned int size,
		unsigned int bytes);
void free_aligned(aligned_pointer_t * ptr);

typedef void (*iteration_loop_t)(lame_global_flags const * gfp, FLOAT pe[2][2],
...
#define MAX_HEADER_LEN 40    /* max size of header is 38 */
	...
	void (*init_xrpow_core)(gr_info * const cod_info, FLOAT xrpow[576],
			int upper, FLOAT * sum);
...
void fill_buffer(lame_global_flags const *gfp, sample_t * mfbuf[2],
		sample_t const *in_buffer[2], int nsamples, int *n_in, int *n_out);

int fill_buffer_resample(lame_global_flags const *gfp, sample_t * outbuf,
		int desired_len, sample_t const *inbuf, int len, int *num_used,
		int channels);
...
int hip_decode1_unclipped(hip_t hip, unsigned char *buffer, size_t len,
